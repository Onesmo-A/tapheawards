        ...
        try {
            $validated = $request->validate([
                'suggested_nominee_name' => [
                    ...
                    'string',
                    'max:255',
                    // Uthibitisho wa kipekee kwa kategooria, device na jina la nominee.
                    Rule::unique('suggestions')->where(function ($query) use ($request) {
                        return $query->where('category_id', $request->category_id)
                                     ->where('fingerprint_js', $request->fingerprint_js)
                                     ->whereRaw('LOWER(TRIM(suggested_nominee_name)) = ?', [Str::lower(trim($request->suggested_nominee_name))]);
                    }),
                ],
