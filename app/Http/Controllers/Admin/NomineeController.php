...
class NomineeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $allowedPerPage = ['10', '50', '100', 'all'];
        if (! in_array($perPage, $allowedPerPage, true)) {
            $perPage = '10';
        }

        $query = Nominee::with('category')->withCount('votes')
        ...
        return Inertia::render('Admin/Nominees/Index', [
            'nominees' => $query->latest()->paginate((int) $perPage)->withQueryString(),
            'filters' => $filters,
        ]);
    }

    public function toggleSuspension(Nominee $nominee)
    {
        $nominee->update([
            'is_suspended' => ! $nominee->is_suspended,
        ]);

        return back()->with('success', $nominee->is_suspended ? 'Nominee suspended successfully.' : 'Nominee reactivated successfully.');
    }
