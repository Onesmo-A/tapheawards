...
class NomineeApplicationController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $allowedPerPage = ['15', '50', '100', 'all'];
        if (! in_array($perPage, $allowedPerPage, true)) {
            $perPage = '15';
        }

        $query = NomineeApplication::query()
            ->with(['user:id,name', 'category:id,name', 'transaction', 'reviewer:id,name'])
        ...
        $applications = $query->latest()->paginate((int) $perPage)->withQueryString();
