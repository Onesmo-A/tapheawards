    ...
    <DefaultLayout>
        <div class="bg-black text-white min-h-screen">
            ...
            <div class="mt-6 flex flex-wrap items-center justify-center gap-3 text-sm text-gray-300">
                <span v-if="season.event_date" class="rounded-full border border-white/10 px-4 py-2">Event date: {{ season.event_date }}</span>
                <span v-if="season.event_location_name" class="rounded-full border border-white/10 px-4 py-2">{{ season.event_location_name }}</span>
                <a v-if="season.event_location_map_url" :href="season.event_location_map_url" target="_blank" rel="noopener" class="rounded-full border border-white/10 px-4 py-2 text-yellow-300 hover:bg-white/5">Open map</a>
            </div>
        </div>
            </header>
