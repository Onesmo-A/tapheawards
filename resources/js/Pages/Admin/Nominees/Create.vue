...
const getStoredImageUrl = (path) => {
  if (!path) return '';
  if (path.startsWith('http')) return path;

  const normalized = path.replace(/^\/+/, '');
  if (normalized.startsWith('file/storage/')) return `/${normalized}`;
  if (normalized.startsWith('storage/')) return `/file/${normalized}`;

  return `/file/storage/${normalized}`;
};
    ...
    <form @submit.prevent="submit" class="space-y-6 bg-gray-800 p-6 rounded-lg shadow-lg">
      ...
      <div>
        ...
        <div v-if="form.image_path && !form.image" class="mb-4">
            ...
            <img :src="getStoredImageUrl(form.image_path)" alt="Current Nominee Image" class="h-32 w-32 rounded-md object-cover border-2 border-gray-600">
