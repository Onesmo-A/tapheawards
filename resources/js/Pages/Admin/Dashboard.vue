...
const props = defineProps({
  ...
  voteAuditWarnings: { type: Object, default: () => ({}) },
  topSuspiciousIps: { type: Array, default: () => [] },
});
...
const awardSeasonContext = computed(() => page.props.awardSeasonContext || { activeYear: null, activeSeason: null, seasons: [] });
const seasonChoices = computed(() => {
...
const seasonForm = useForm({
  active_season_year: awardSeasonContext.value.activeYear || '',
});

watch(
  () => awardSeasonContext.value.activeYear,
  (value) => {
    seasonForm.active_season_year = value || '';
  }
);
...
const switchSeason = () => {
  seasonForm.patch(route('admin.settings.update-season'), {
    preserveScroll: true,
  });
};
</script>
  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
      ...
      <div class="mt-4 flex flex-wrap items-center gap-3 sm:mt-0">
        <div class="min-w-[320px] rounded-2xl border border-white/10 bg-gray-900/70 px-4 py-3">
          <p class="text-xs uppercase tracking-wide text-gray-400">Active Season</p>
          <div class="mt-2 flex flex-col gap-3 sm:flex-row sm:items-center">
            <select
              v-model="seasonForm.active_season_year"
              class="w-full rounded-xl border border-white/10 bg-gray-950 px-3 py-3 text-sm text-white sm:w-72"
              :disabled="seasonChoices.length === 0"
            >
              <option v-for="season in seasonChoices" :key="season.id || season.year" :value="season.year">
            ...
            <button
              type="button"
              @click="switchSeason"
              class="inline-flex items-center rounded-xl bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-500"
              :disabled="seasonForm.processing || !seasonForm.active_season_year"
            >
              Switch
            </button>
          </div>
          <p class="mt-2 text-xs text-gray-500">
            Unaweza kubadili active year hata kabla ya kuunda season record mpya.
          </p>
        </div>
        <button @click="refreshData" class="inline-flex items-center rounded-xl bg-white/10 px-4 py-3 text-sm font-semibold text-white transition hover:bg-white/20">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-8 gap-6">
      ...
      <StatCard title="Successful Vote Attempts" :value="stats.successful_attempts" :href="route('admin.vote-attempt-logs.index', { status: 'success' })" color="emerald">
        <CheckCircleIcon class="h-6 w-6" />
      </StatCard>
