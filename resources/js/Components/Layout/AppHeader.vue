...
const headerScrollThreshold = 40;
const handleScroll = () => {
  isScrolled.value = !isHomePage.value || window.scrollY > headerScrollThreshold;
};
 ...
 <header
  :class="[
    ...
    isHomePage && !isScrolled
      ? 'bg-transparent backdrop-blur-md border-b border-white/10 shadow-none'
      : 'bg-white/92 backdrop-blur-xl shadow-lg border-b border-red-500/10',
  ]"
  ...
  >
      ...
      <div class="flex items-center justify-between h-20">
        ...
        <nav class="hidden lg:flex items-center space-x-1 ml-auto">
          <Link
            ...
            :key="item.name"
            :href="item.href"
            :class="isHomePage && !isScrolled ? 'px-3 py-2 font-medium transition text-white hover:text-red-200' : 'px-3 py-2 font-medium transition text-[var(--text-primary)] hover:text-[var(--text-accent)]'"
          >
            {{ item.name }}
          </Link>
          ...
          <button
            ...
            :class="isHomePage && !isScrolled ? 'group inline-flex items-center px-3 py-2 font-medium transition text-white hover:text-red-200' : 'group inline-flex items-center px-3 py-2 font-medium transition text-[var(--text-primary)] hover:text-[var(--text-accent)]'"
          >
            <span>Awards</span>
        ...
        <div class="hidden lg:flex items-center space-x-4 ml-6">
          <template v-if="isAuthenticated && user">
            <span :class="isHomePage && !isScrolled ? 'text-base font-medium text-white' : 'text-base font-medium text-[var(--text-primary)]'">Habari, {{ user.name.split(' ')[0] }}!</span>
            <button @click="handleLogout" class="btn-primary !text-sm !px-4 !py-2">Log Out</button>
          </template>
          <template v-else>
            <Link :href="route('login')" class="btn-primary text-base font-medium transition">Sign In</Link>
        ...
        <div class="flex lg:hidden">
          <button
            ...
            :class="isHomePage && !isScrolled ? 'inline-flex items-center justify-center p-2 rounded-md text-white hover:text-red-200 hover:bg-white/10 focus:outline-none' : 'inline-flex items-center justify-center p-2 rounded-md text-[var(--text-primary)] hover:text-[var(--text-accent)] hover:bg-gray-100 focus:outline-none'"
          >
    ...
    <transition name="fade">
      <div
        ...
        class="lg:hidden panel-red-glass mx-3 rounded-3xl px-2 pt-2 pb-3 space-y-1 sm:px-3 border-t border-red-300/20 shadow-lg"
      >
        <Link
          v-for="item in navigation"
          :key="item.name"
          ...
          class="block px-3 py-2 rounded-md text-base font-medium text-white/90 hover:text-white hover:bg-white/10"
        >
          {{ item.name }}
        </Link>
        ...
        <div>
          <button
            ...
            class="w-full flex items-center justify-between px-3 py-2 rounded-md text-base font-medium text-white/90 hover:text-white hover:bg-white/10"
          >
            <span>Awards</span>
          ...
          <div v-show="mobileSubMenuOpen === 'awards'" class="pt-2 pb-1 pl-5 space-y-1">
            <Link
              ...
              :href="item.href"
              @click="closeMobileMenu"
              class="block px-3 py-2 rounded-md text-sm font-medium text-white/70 hover:text-white hover:bg-white/10"
            >
              {{ item.name }}
            </Link>
          </div>
        ...
        <!-- Auth buttons -->
        <div class="border-t border-white/10 pt-4 pb-3 space-y-2">
          <template v-if="isAuthenticated && user">
            <span class="block px-3 py-2 rounded-md text-base font-medium text-white">Habari, {{ user.name.split(' ')[0] }}!</span>
            <button
              @click="handleLogout"
              class="block w-full text-left px-3 py-2 rounded-md text-base font-medium text-white/90 hover:text-white hover:bg-white/10"
            >
              Log Out
            </button>
          </template>
          <template v-else>
            <Link
              :href="route('login')"
              @click="closeMobileMenu"
              class="block btn-primary px-3 py-2 rounded-md text-base font-medium text-white"
            >
              Sign In
            </Link>
            <Link
              :href="route('register')"
              @click="closeMobileMenu"
              class="block rounded-md border border-white/15 bg-white/5 px-3 py-2 text-base font-medium text-white hover:bg-white/10"
            >
              Sign Up
            </Link>
          </template>
    ...
    <transition name="fade">
      <div v-show="isAwardsDropdownOpen" class="absolute left-0 top-20 w-screen z-40">
        <div class="panel-red-glass mx-4 rounded-3xl border-t-4 border-red-400/60">
          <div class="max-w-7xl mx-auto px-6 py-10 grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-10">
            <div v-for="column in awardsMegaMenu" :key="column.title">
              <p class="font-bold text-base text-white/70 uppercase tracking-wider mb-4">{{ column.title }}</p>
              <ul class="space-y-4">
                <li v-for="item in column.links" :key="item.name">
                  ...
                  >
                    <p class="font-semibold text-white text-lg group-hover:text-red-200">{{ item.name }}</p>
                    <p class="text-sm text-white/70 mt-1">{{ item.description }}</p>
                  </Link>
                </li>
              </ul>
            </div>
