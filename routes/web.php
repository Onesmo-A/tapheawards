...
use App\Http\Controllers\Admin\SeasonAwardController as AdminSeasonAwardController;
use App\Http\Controllers\Admin\VoteAttemptLogController as AdminVoteAttemptLogController;
use App\Http\Controllers\Admin\VoteArchiveController as AdminVoteArchiveController;
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::middleware(['admin'])->prefix('admin')->name('admin.')->group(function () {
        ...
        Route::patch('/categories/{category}/toggle-voting', [AdminCategoryController::class, 'toggleVoting'])->name('categories.toggle-voting');
        ...
        Route::patch('/nominees/{nominee}/toggle-suspension', [AdminNomineeController::class, 'toggleSuspension'])->name('nominees.toggle-suspension');
        ...
        Route::patch('/settings/season', [AdminSettingController::class, 'updateSeason'])->name('settings.update-season');
        ...
        Route::resource('hero-banners', AdminHeroBannerController::class)->except(['create', 'edit', 'show'])->names('hero-banners');
        Route::resource('sponsors', AdminSponsorController::class)->except(['show'])->names('sponsors');
        Route::resource('season-awards', AdminSeasonAwardController::class)->except(['show'])->names('season-awards');
        Route::get('/vote-attempt-logs', [AdminVoteAttemptLogController::class, 'index'])->name('vote-attempt-logs.index');
        Route::match(['delete', 'post'], '/vote-attempt-logs/prune', [AdminVoteAttemptLogController::class, 'prune'])->name('vote-attempt-logs.prune');
        Route::get('/vote-archives', [AdminVoteArchiveController::class, 'index'])->name('vote-archives.index');
        Route::post('/vote-archives/snapshot', [AdminVoteArchiveController::class, 'snapshot'])->name('vote-archives.snapshot');
...
// ========== Public Routes ==========
Route::get('/file/storage/{path}', [StorageController::class, 'serve'])
    ->where('path', '.*')
    ->name('storage.file');
...
Route::get('/nominees/{nominee}', [\App\Http\Controllers\NomineeController::class, 'show'])->name('nominees.show');
...
Route::get('/awards/archives', [AwardsController::class, 'archives'])->name('awards.archives');
Route::get('/awards/seasons/{seasonAward}', [AwardsController::class, 'showSeason'])->name('seasonAwards.show');
...
// Rate limiting sasa inafanywa ndani ya controller/service ili iweze kuandikwa
// kwenye vote attempt logs kabla ya kurudisha 429.
Route::get('/nominees/{nominee}/vote-session', [VoteController::class, 'voteSession'])
    ->name('nominees.vote-session');
